...
namespace Urok11TSLAB.ViewModels
{
    ...
    public class Candle
    {
        ...
        public void SetCandleFromString(string In)
        {
            ...
            Open = Convert.ToDecimal(sIn[2].Replace('.', ','));
            High = Convert.ToDecimal(sIn[3].Replace('.', ','));
            Low = Convert.ToDecimal(sIn[4].Replace('.', ','));
            Close = Convert.ToDecimal(sIn[5].Replace('.', ','));

            try
            {
                Volume = Convert.ToDecimal(sIn[6].Replace('.', ','));
            }
            catch (Exception)
            {
                Volume = 1;
